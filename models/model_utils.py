...
def model_set_attr(config, param_dict):
    for param, _ in param_dict.items():
        if hasattr(config, param):
            param_dict[param] = getattr(config, param)
    return param_dict
...
class AvgPoolHINet2d(nn.Module):
...
class Local_Base():
    def convert(self, *args, train_size, **kwargs):
        ...
        pe = torch.rand(1, 2, 256, 256)
        with torch.no_grad():
            self.forward([imgs, pe])
        # with torch.no_grad(): #1, 3, 256,256
        #    self.forward(imgs)
